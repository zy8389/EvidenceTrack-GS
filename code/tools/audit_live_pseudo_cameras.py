...
from tools.audit_pseudo_camera_manifest import audit_live_scene
...
def main() -> None:
    ...
    try:
        ...
        if current_commit != PINNED_UPSTREAM_COMMIT:
            raise RuntimeError(
                "Live pseudo-camera gate is running on the wrong upstream commit: "
                f"current={current_commit}, expected={PINNED_UPSTREAM_COMMIT}"
            )
        ...
        result.update(
            {
                ...
                "upstream_commit_current": current_commit,
