...
def dino_metadata():
    return {
        ...
        "extractor_provenance": {
            "backend": "dinov2:dinov2_vits14",
            "local_repository_path": "/opt/dinov2",
            "repository_commit": "a" * 40,
            "pretrained_weight_path": "/opt/weights/dinov2_vits14.pth",
...
def test_report_protocol_contains_immutable_dinov2_identity():
    protocol = identity_protocol_from_metadata(dino_metadata(), 32)
    assert protocol["feature_backend_identity"] == "dinov2:dinov2_vits14"
    assert protocol["dinov2_repository_commit"] == "a" * 40
    assert protocol["dinov2_pretrained_weight_sha256"] == "b" * 64


def test_missing_or_malformed_dinov2_weight_hash_fails_closed():
    ...
    with pytest.raises(ValueError, match="pretrained_weight_sha256"):
        identity_protocol_from_metadata(metadata, 32)
