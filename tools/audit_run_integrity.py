...
from tools.aggregate_results import _require_row_bound_pair_audits
from tools.audit_pseudo_camera_manifest import audit as audit_pseudo_manifest
from tools.compare_identity_reports import compare_reports, read as read_identity_report
from tools.evaluate_track_evidence import (
    load_paired_target_manifest,
    validate_paired_manifest_metadata,
)
from tools.metrics_from_log import extract_metric_row
...
def validate_environment_binding(
    ...
    if (
        environment.get("git_commit") != PINNED_UPSTREAM_COMMIT
        or details.get("current_commit") != PINNED_UPSTREAM_COMMIT
        or details.get("expected_commit") != PINNED_UPSTREAM_COMMIT
    ):
        raise ValueError("Environment report/gate is not bound to the pinned upstream commit")
    ...
    protocol_candidates = [
        code_root / "research_scripts" / "run_stage.sh",
        code_root / "scripts" / "run_stage.sh",
        code_root / "configs" / "controlled_protocol.json",
        *sorted(code_root.glob("requirements-*.txt")),
    ]
