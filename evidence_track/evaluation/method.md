...
您的`env/train.yaml`中有`lightglue==0.0`，这可能导致问题。建议：

```bash
# 测试当前依赖
python evidence_track/evaluation/test_dependencies.py
...
```bash
# 使用修复版本的脚本
python evidence_track/evaluation/extract_and_triangulate_tracks_fixed.py -s /path/to/your/scene
...
### 3. 验证和可视化结果
```bash
python evidence_track/evaluation/verify_and_visualize_fixed.py -s /path/to/your/scene
```
