...
\newcommand{\draftversion}{Archived initial draft v1}
...
\begin{document}
\maketitle
\begin{center}\small\textbf{Pre-experiment extended manuscript.}\quad 6 September 2026.\\
...
\begin{abstract}
Diffusion-enhanced novel views may be visually compelling without preserving the identity or location of scene structure. In sparse-view Gaussian splatting, this ambiguity is compounded when feature tracks inherit held-out observations, uncertainty depends on arbitrary world scale, or a purported diffusion gain is actually an effect of additional optimization. We develop an identity-controlled research protocol around a repaired track-regularized Gaussian baseline and a frozen single-step diffusion cache. Track membership, triangulated anchors, initialization colors, and confidence are constructed from source RGB only, conditional on supplied camera calibration. A shared 10,000-step checkpoint branches into matched 2,000-step continuations with and without pseudo-view RGB supervision. A separate diagnostic compares source-track queries on rendered, diffusion-enhanced, and real held-out images against projection-only and identity-corrupted controls. We derive local source-geometry observability, similarity-equivariant confidence, and a conditional-information argument explaining why generated views are not independent scene measurements. These results justify protocol design, not global convergence or the truth of hallucinated content. We also specify failure-aware metrics, camera-coordinate conventions, and scene-level paired inference. In the available CPU environment, eleven inherited smoke tests and twenty-three additional regression tests pass; an isolated three-camera point-recovery test reduces mean reprojection error from 9.7511 to 0.0276 pixels. No real-scene reconstruction or diffusion-model inference result is asserted. The accompanying implementation and preregistered experiments are designed to determine whether diffusion-assisted gains actually survive geometry and identity controls before any stronger method claim is made.
\end{abstract}
...
The research has three coupled parts. First, a repaired source-track baseline makes the origin of every anchor explicit and makes the geometric loss act on current Gaussian centers. Second, a matched continuation isolates the effect of adding a fixed frozen pseudo-view cache. Third, an evaluation-only identity diagnostic tests whether source-specific descriptors help beyond merely searching near a projected location. These parts are implemented without a new learned adapter, a learned reliability network, or Bayesian posterior fusion. The diagnostic does not feed held-out targets back into training.
...
\caption{Implemented scope and the evaluation boundary. Source RGB and supplied calibration construct track identity and the geometric baseline. A fixed A0 checkpoint yields a frozen pseudo-view cache and the two continuations A1/B. Held-out RGB is used only for final image evaluation and correspondence diagnostics. Dashed arrows are evaluation-only; the diagnostic is not an implemented feedback module.}
\label{fig:pipeline}
\end{figure*}
...
\section{Frozen Diffusion and the Controlled Continuation}
Let $\theta_0$ denote the complete 10,000-step A0 state. From A0, render a source-derived pool of 32 pseudo cameras and choose reference images only from the source set. A fixed reference-conditioned Difix model generates
\begin{equation}
\widetilde I_v=D_W(\mathcal R(\theta_0,C_v),I_{r(v)};z),
\end{equation}
...
A1 resumes ordinary source/track training from A0. B resumes from the exact same serialized A0 state, adding only the frozen pseudo RGB loss:
\begin{align}
\cL_{A1}&=\cL_{\mathrm{src}}+\lambda_t r_t(t)\cL_{\mathrm{trk}},\\
\cL_B&=\cL_{A1}+g(t)\lambda_p r_p(t)\cL_{\mathrm{pseudo}},\\
\cL_{\mathrm{pseudo}}&=0.8\norm{\mathcal R(\theta,C_v)-\widetilde I_v}_1
+0.2(1-\mathrm{SSIM}).
\label{eq:lossb}
\end{align}
The source term is the inherited RGB photometric objective. The strict controlled revision disables legacy monocular-depth supervision, legacy pseudo-depth supervision, the optional legacy geometry regularizer, and learned appearance adapters. The repaired track coefficient is $\lambda_t=0.1$. The first experiment uses full precision and freezes topology at 10,000 steps for both continuations. These choices make the tested objective and branch comparison identifiable; later experiments can relax them symmetrically.

For B, $g(t)=1$ iff $10000<t<12000$ and $t\bmod50=0$; $r_p(t)=\min(1,(t-10000)/500)$ and $\lambda_p=0.1$. There are exactly 39 scheduled pseudo calls, cycling through all 32 views. The logged count and unique-view coverage, rather than only the command-line settings, determine whether the protocol was realized. Pseudo calls are interleaved with source updates, not 32 new real observations.

\paragraph{Matched state and randomization.} Checkpoints include model tensors, Gaussian-associated auxiliary state, Adam state, and random-generator states. They are written \emph{after} the update and reset for the declared iteration, and the final step is not skipped. Legacy incomplete tuples are rejected for the revised controlled protocol. Source-camera sampling uses its own deterministic sequence, independent of pseudo sampling. A1/B must have identical initial checkpoint hashes and real-view sequences; their final weights are expected to differ because their gradients differ. An offline cache is the treatment; extra training is controlled by A1, not by comparing B only with A0.
...
\begin{figure}[t]
\centering\input{figures/identity_controls.tex}
...
The initial reporting plan summarized mean error over successful matches and PCK at thresholds $3,5,8,16$ pixels. It did not yet specify a common failed-match denominator. This v1 snapshot records that omission for the internal review; it must not be used for the revised main result.

The initial aggregation plan used scene averages but had not finalized a failed-prediction penalty or a strict complete-pair validation rule. Those are explicit open protocol decisions in this archived version.
...
\textbf{H2, actual diffusion utility.} At 12,000 updates, B should outperform A1 on preregistered held-out reconstruction metrics without an unacceptable geometry regression. LPIPS is a perceptual metric, not a direct surface measurement. A meaningful main claim requires a favorable paired confidence interval on the primary image metric and independent geometry evidence; PSNR and SSIM are supporting outcomes. No threshold is retrofitted to make a negative result pass.

\textbf{H3, identity-specific geometry.} Difix-target matching should improve over GS-target matching and projection-only prediction while retaining positive valid hard-negative identity margin. These differences are computed on common paired support with failure-aware denominators. A visual improvement alone cannot satisfy H3.
...
A0 is the shared 10k checkpoint, A1 the 12k source-only continuation, and B the 12k continuation with frozen Difix pseudo RGB. A separate source-initialized renderer without the track term isolates repaired geometry. A reconstruction trained against its own unchanged pseudo renders controls for extra rendering/supervision activity. A simple non-generative image enhancement control is planned to distinguish generic sharpening from a diffusion-specific mechanism.
...
\paragraph{Measured CPU checks.} The available environment is Python 3.13.5, PyTorch 2.10.0+cpu, NumPy 2.3.5, and OpenCV 4.13.0. CUDA is unavailable. Eleven Phase-2.1 smoke checks and twenty-three new regression tests pass. The Difix reproducibility smoke check uses a seeded mock pipeline; it does not show that the real diffusion model is deterministic on a GPU. The new tests include half-pixel transforms, calibrated NDC algebra, covariance scale changes, degeneracy rejection, failure denominators, paired-result validation, an Adam continuation fixture, and AST integration fixtures. A complete real upstream checkout and CUDA extension were not executed in this environment.
...
\bottomrule
\end{tabular}
\caption{Actual CPU measurement. One perturbed point, exact synthetic source observations, fixed cameras, Adam step size 0.01. This is not a Gaussian-rasterizer benchmark or real-scene reconstruction experiment. Full-precision values and the test script are in the artifact.}
\label{tab:synthetic}
\end{table}
...
\midrule
LLFF & A0/A1/B & \NR & \NR & \NR & \NR\\
DTU & A0/A1/B & \NR & \NR & \NR & \NR\\
Mip360 & A0/A1/B & \NR & \NR & \NR & \NR\\
\bottomrule
\end{tabular}
...
\section{Conclusion}
We have turned a broad ``diffusion plus tracks'' idea into a falsifiable, source-provenanced reconstruction protocol and an identity-controlled diagnostic. The theory supports local geometric observability, coordinate-consistent confidence, and careful interpretation of generated supervision. The implementation has passed the reported CPU checks, but efficacy and novelty remain empirical questions. The next decisive evidence is a matched A1/B continuation together with failure-aware, identity-sensitive held-out diagnostics and independent geometry. Stronger architectural claims should follow those results, not precede them.
...
CPU gate & \texttt{scripts/run\_cpu.sh} & Smoke logs, 23 regression results, synthetic recovery JSON, environment record. No claim of real diffusion inference.\\
...
Continue & \texttt{A1}, then \texttt{B} & Same initial checkpoint; equal source sequence and update count; B logs 39 pseudo calls / 32 unique views.\\
Diagnostic & \texttt{evidence-export}, \texttt{evidence-cache}, \texttt{evidence}, \texttt{report} & Common paired track CSV, feature metadata, unconditional PCK, failure rate, geometry-relative gain and hard-negative validity.\\
...
\section{Initial Submission Checklist}
The v2 draft separates the real-reference diagnostic from a bound, distinguishes descriptor identity from geometric improvement, changes main metrics to failure-aware paired reporting, and narrows mathematical claims to their assumptions. It also makes source-only conditioning on supplied poses explicit and rejects incomplete continuation checkpoints. This initial snapshot is the input to the accompanying simulated review exercise. They are internal analytic reviews by one assistant, not external peer review, and they do not certify acceptance.
