...
The assembly order was fixed as cumulative pre-AST patch, source overlay, then guarded AST integration. The cumulative patch explicitly deletes `geometric_constraints/adaptive_weighting.py`; that file must remain absent unless a future research change deliberately reintroduces and validates it.
...
The assembly marker says validation was not run by the assembler itself. Post-assembly validation was run separately; the corresponding report and logs are under `validation/`.
