#!/usr/bin/env bash
# Execute inside the materialized full-source checkout. Every gate fails closed.
set -euo pipefail
...
marker_upstream = marker.get("upstream_commit")
checks["pinned_upstream_commit"] = marker_upstream == expected_commit
checks["environment_report_commit"] = bool(current_commit) and environment.get("git_commit") == current_commit
details["current_commit"] = current_commit
details["expected_upstream_commit"] = expected_commit
details["marker_upstream_commit"] = marker_upstream
...
if environment.get("upstream_commit") != expected_commit or details.get("marker_upstream_commit") != expected_commit:
    raise SystemExit("Environment gate is not bound to the pinned upstream provenance; rerun env")
if not details.get("current_commit") or environment.get("git_commit") != details.get("current_commit"):
    raise SystemExit("Environment gate is not bound to the current repository revision; rerun env")
root = marker.parent
recorded_python = {
    str(relative).replace("\\", "/"): str(digest)
    for relative, digest in environment.get("python_sha256", {}).items()
...
audit_pair_self() {
  ...
  run_json_gate "$RUN/a1_selfrender_pair_audit.json" "A1/SelfRender controlled-pair gate" \
    python evidence_track/evaluation/audit_controlled_pair.py --baseline "$A1" --treatment "$SELF" --require-pseudo --dataset "$DATASET" --scene "$SCENE_NAME" --output "$RUN/a1_selfrender_pair_audit.json"
}

audit_pair_b() {
  require_recovery_gate
  run_json_gate "$RUN/a1_b_pair_audit.json" "A1/B controlled-pair gate" \
    python evidence_track/evaluation/audit_controlled_pair.py --baseline "$A1" --treatment "$B" --require-pseudo --dataset "$DATASET" --scene "$SCENE_NAME" --output "$RUN/a1_b_pair_audit.json"
}
...
audit_pair_pseudo() {
  ...
  run_json_gate "$RUN/selfrender_b_pseudo_control_audit.json" "SelfRender/B pseudo-control gate" \
    python evidence_track/evaluation/audit_pseudo_control_pair.py --self-render "$SELF" --b "$B" --self-render-manifest "$RUN/pseudo/self_render_manifest.jsonl" --b-manifest "$PSEUDO_MANIFEST" --dataset "$DATASET" --scene "$SCENE_NAME" --output "$RUN/selfrender_b_pseudo_control_audit.json"
}
...
generate_contrast_metrics() {
  ...
  python evidence_track/evaluation/build_contrast_metrics.py --baseline A1 --baseline-log "$A1/train.log" --treatment B --treatment-log "$B/train.log" --pair-audit "$RUN/a1_b_pair_audit.json" --dataset "$DATASET" --scene "$SCENE_NAME" --seed "$SEED" --output "$staging/a1_b.csv"
  python evidence_track/evaluation/build_contrast_metrics.py --baseline A1 --baseline-log "$A1/train.log" --treatment SelfRender --treatment-log "$SELF/train.log" --pair-audit "$RUN/a1_selfrender_pair_audit.json" --dataset "$DATASET" --scene "$SCENE_NAME" --seed "$SEED" --output "$staging/a1_selfrender.csv"
  python evidence_track/evaluation/build_contrast_metrics.py --baseline SelfRender --baseline-log "$SELF/train.log" --treatment B --treatment-log "$B/train.log" --pair-audit "$RUN/selfrender_b_pseudo_control_audit.json" --dataset "$DATASET" --scene "$SCENE_NAME" --seed "$SEED" --output "$staging/selfrender_b.csv"
...
refresh_identity_association() {
  ...
  run_json_gate "$IDENTITY_ASSOCIATION" "A1/B identity association gate" \
    python evidence_track/evaluation/compare_identity_reports.py --a1 "$RUN/identity_a1/paired_report.json" --b "$RUN/identity_b/paired_report.json" --output "$IDENTITY_ASSOCIATION"
}
...
case "$STAGE" in
  env)
    ...
    python evidence_track/evaluation/collect_environment.py --output "$ENVIRONMENT_REPORT"
    require_json_object "$ENVIRONMENT_REPORT" "environment report"
    write_environment_gate
    ;;
  tracks)
  ...
  projection)
    ...
    run_json_gate "$PROJECTION_GATE" "H5/live-Scene projection gate" \
      python evidence_track/evaluation/check_camera_projection_equivalence.py "${COMMON[@]}" -m "$PROJECTION_MODEL" --track-h5 "$TRACK" --output "$PROJECTION_GATE"
  ...
  pseudo-live)
    ...
    run_json_gate "$PSEUDO_LIVE_GATE" "live pseudo-camera provenance gate" \
      python evidence_track/evaluation/audit_live_pseudo_cameras.py "${COMMON[@]}" -m "$PSEUDO_LIVE_MODEL" --output "$PSEUDO_LIVE_GATE" --seed "$SEED"
    python - "$PSEUDO_LIVE_GATE" "$PROJECTION_GATE" <<'PY'
import json
import sys
from pathlib import Path
...
PY
  ...
  recover)
    ...
    run_json_gate "$RUN/recovery.json" "real Gaussian recovery gate" \
      python evidence_track/evaluation/test_geometry_recovery.py "${COMMON[@]}" --strict_tracks -m "$RUN/recovery_scratch" --checkpoint "$CKPT" --steps 100 --perturbation-ratios 0.005 0.01 0.02 --experiment_seed "$SEED" --output "$RUN/recovery.json"
    require_recovery_gate
    ;;
  export)
    require_projection_gate
    ...
    python evidence_track/evaluation/export_pseudo_views.py "${COMMON[@]}" -m "$A0" --iteration 10000 --a0-checkpoint "$CKPT" --live-pseudo-audit "$PSEUDO_LIVE_GATE" --output-dir "$RUN/pseudo" --max-pseudo-views 32 --seed "$SEED"
  ...
  pseudo-audit)
    ...
    run_json_gate "$PSEUDO_GATE" "pseudo-camera provenance gate" \
      python evidence_track/evaluation/audit_pseudo_camera_manifest.py --manifest "$PSEUDO_MANIFEST" --output "$PSEUDO_GATE"
    stamp_pseudo_gate
    require_pseudo_gate
    ;;
  self-manifest)
    ...
    python evidence_track/evaluation/make_self_render_manifest.py --input-manifest "$PSEUDO_MANIFEST" --output-manifest "$RUN/pseudo/self_render_manifest.jsonl"
    ;;
  cache)
    require_pseudo_gate
    require_recovery_gate
    need_difix
    python evidence_track/evaluation/run_difix_cache.py --manifest "$PSEUDO_MANIFEST" --difix-repo "$DIFIX_REPO" --model-id nvidia/difix_ref --model-revision "$DIFIX_MODEL_REVISION" --dtype fp16 --timestep 199 --guidance-scale 0 --seed "$SEED" --reproducibility-check
  ...
  evidence-export-a1)
    ...
    python evidence_track/evaluation/export_heldout_views.py "${COMMON[@]}" -m "$A1" --iteration 12000 --checkpoint "$A1/chkpnt12000.pth" --dataset "$DATASET" --pair-audit "$RUN/a1_b_pair_audit.json" --output-dir "$RUN/evidence_a1" --seed "$SEED"
    ;;
  evidence-export-b)
    audit_pair_b
    reset_run_subdir "$RUN/evidence_b" "B held-out evidence directory"
    python evidence_track/evaluation/export_heldout_views.py "${COMMON[@]}" -m "$B" --iteration 12000 --checkpoint "$B/chkpnt12000.pth" --dataset "$DATASET" --pair-audit "$RUN/a1_b_pair_audit.json" --output-dir "$RUN/evidence_b" --seed "$SEED"
    ;;
  evidence-cache-a1|evidence-cache-b)
    ...
    python evidence_track/evaluation/run_difix_cache.py --manifest "$E/difix_manifest.jsonl" --difix-repo "$DIFIX_REPO" --model-id nvidia/difix_ref --model-revision "$DIFIX_MODEL_REVISION" --dtype fp16 --timestep 199 --guidance-scale 0 --seed "$SEED" --reproducibility-check
  ...
  identity-manifest)
    ...
    python evidence_track/evaluation/make_paired_identity_manifest.py --a1-manifest "$RUN/evidence_a1/evidence_manifest.jsonl" --b-manifest "$RUN/evidence_b/evidence_manifest.jsonl" --output "$PAIRED_IDENTITY_MANIFEST"
    require_paired_identity_manifest
    ;;
  identity-a1|identity-b)
    ...
    python evidence_track/evaluation/evaluate_track_evidence.py --track-h5 "$TRACK" --images-dir "$SCENE/images" --target-manifest "$PAIRED_IDENTITY_MANIFEST" --paired-arm "$ARM" --feature-backend dinov2 --dinov2-model dinov2_vits14 --window-radii 16 32 48 --primary-radius 32 --shift-pixels 4 8 16 --hard-negative-radius 32 --temperature 0.07 --output-dir "$OUT"
    python evidence_track/evaluation/paired_evidence_report.py --csv "$OUT/per_track.csv" --radius 32 --output "$OUT/paired_report.json"
    ;;
  identity-report)
    refresh_identity_association
    ;;
  report)
    ...
    python evidence_track/evaluation/paired_evidence_report.py --csv "$RUN/identity_a1/per_track.csv" --radius 32 --output "$RUN/identity_a1/paired_report.json"
    python evidence_track/evaluation/paired_evidence_report.py --csv "$RUN/identity_b/per_track.csv" --radius 32 --output "$RUN/identity_b/paired_report.json"
    refresh_identity_association
    ;;
  metrics)
    require_completed_training_runs
  ...
  integrity)
    ...
    run_json_gate "$INTEGRITY_REPORT" "single-scene/seed final integrity gate" \
      python evidence_track/evaluation/audit_run_integrity.py --run "$RUN" --dataset "$DATASET" --scene "$SCENE_NAME" --seed "$SEED" --output "$INTEGRITY_REPORT"
    ;;
  *)
